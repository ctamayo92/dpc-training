...
void runSharedMemory(
    ...
    std::memcpy(c_shared, c_matrix.data(), N*N * sizeof(int));
...
// // ND-Range Kernel
    ...
    // Hierarchical parallelism
    // range<2> num_group(N/B, N/B);
    // range<2> group_size(B, B);

    // h.parallel_for_work_group<class matrix_mul>(num_groups, [=](group<2> grp){
    //     int jb = grp.get_id(0);
    //     int ib = grp.get_id(1);
    //     grp.parallel_for_work_item(group_size, [&](h_item<2> item){
